...
class Camera():
    ...
    def set_picture_size(self, width:int=2560, height:int=720):
...
def find_calc_shapes(pic1, pic2):
    pic, cord, s1 = contour_img(pic1)
    pic2, cord2, s2 = contour_img(pic2)
    if len(cord) == 2 and len(cord2) == 2:
        distance = calc_distance([cord, cord2], 60, 6)
        if distance > 10:
            cv2.putText(pic, f'Distance:{distance} cm',cord, cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,0), 3, cv2.LINE_AA)
            cv2.putText(pic, f'Distance:{distance} cm',cord, cv2.FONT_HERSHEY_SIMPLEX, 1, (255,255,255), 2, cv2.LINE_AA)
            width  = calc_size (s1[0],(cord,cord2) ,  0)
            cv2.putText(pic, f'Width:{width} cm',(cord[0], cord[1]+100), cv2.FONT_HERSHEY_SIMPLEX, 1, (255,255,255), 2, cv2.LINE_AA)
    return pic1
